...
std::unique_ptr<Edge> createEdge(uint32_t begin_node_id, uint32_t end_node_id,
    uint32_t label, int32_t weight) {
    return std::unique_ptr<Edge>(new Edge(begin_node_id, end_node_id, label, weight));
}

Edge::Edge(uint32_t begin_node_id, uint32_t end_node_id, uint32_t label, int32_t weight) :
...
void Edge::add_sequence(uint32_t label, int32_t weight) {
